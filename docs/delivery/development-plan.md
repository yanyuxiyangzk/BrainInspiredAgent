...
bia 交互终端的直接对话能力：复用平台治理 LLM 栈（Adapter + GovernedLlmClient + ConversationService），在 `❯` 提示符下直接输入文字即可多轮对话。R01～R08 已全部完成并通过自动化测试与伪终端（pty + 终端模拟器）黑盒验证。

| ID | 状态 | 负责人 | 依赖 | 交付物/完成标准 | 验收 |
|---|---|---|---|---|---|
| R01 | `✅ 已开发已测试` | BE | I01 | 治理 LLM 契约与多 Provider 接入：OpenAI 兼容/Anthropic 适配、重试与 token 预算；`/model` 交互式配置 Provider/Endpoint/Key/模型并持久化 `.env` | 契约测试；/model 配置回读一致 |
| R02 | `✅ 已开发已测试` | BE | R01 | 终端内多轮对话：ChatSession 会话上下文、`/clear` 清空、模型状态实时显示 | 多轮上下文测试；未配置模型引导文案 |
| R03 | `✅ 已开发已测试` | BE | R02 | SSE 流式打字机输出与空白行规整（WhitespaceNormalizer） | 流式契约测试；输出节奏单空行 |
| R04 | `✅ 已开发已测试` | BE | R02 | 图片输入：剪贴板 Ctrl+V/Alt+V 贴图、Windows 路径归一、`/img` 剪贴板与 `/img <路径> [提问]` 直接提问、缺失路径警告、5MB 上限 | 附图请求含 base64 图像；占位符 `[图片#N]` 回显 |
| R05 | `✅ 已开发已测试` | BE | R03 | 流式 Markdown 渲染：标题/加粗/斜体/删除线/行内码/链接/引用/列表/围栏代码块，行缓冲不重绘 | MarkdownStreamFormatter 契约测试 |
| R06 | `✅ 已开发已测试` | BE/TL | U01 | 输入框常驻与历史保真：PromptHandle 长活应用、回车不退出、回复经 run_in_terminal 滚入框体上方、框体（横线/输入行/状态行/补全菜单）回复期间全程可见、历史零丢失 | pty 终端模拟器多轮验证；44 项 shell/chat 测试 |
| R07 | `✅ 已开发已测试` | BE | R06 | 思考指示器：框体顶线上方 `✶ 思考中… (1m 21s · ↓ 1.8k tokens)`，spinner 每秒轮换、token 估算实时跳动、使用贴士每 10s 轮换，完成后消失 | pty 流式期间抓屏验证 |
| R08 | `✅ 已开发已测试` | BE | R06 | 版面与节奏：欢迎面板单分隔线隔开对话历史、状态行为框体页脚、回复紧贴输入、轮次间单空行、`/model` 嵌套交互时框体收起重建 | pty 抓屏验证；布局无残骸/无空洞 |

里程碑出口：`bia` 一条命令进入终端即可完成模型配置、多轮对话、贴图提问与全部 `/` 命令；对话历史在任何屏幕高度与滚动条件下完整保留。该阶段不改变治理边界：终端对话独立于量化认知闭环，后者仍走 Plan/Grant 治理链。
